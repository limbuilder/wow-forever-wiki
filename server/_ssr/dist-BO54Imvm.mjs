...
var createKyselyAdapter = async (config) => {
	...
	if ("fileControl" in db) {
		const { BunSqliteDialect } = await import("./bun-sqlite-dialect-na--YwnN-DU-5lqWn.mjs");
	...
	if ("createSession" in db) {
		...
		if (DatabaseSync && db instanceof DatabaseSync) {
			const { NodeSqliteDialect } = await import("./node-sqlite-dialect-Co34rNtD.mjs");
			dialect = new NodeSqliteDialect({ database: db });
		}
	}
	if ("batch" in db && "exec" in db && "prepare" in db) {
		const { D1SqliteDialect } = await import("./d1-sqlite-dialect-C2B7YsIT-B06OE-yl.mjs");
