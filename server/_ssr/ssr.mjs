...
import { n as globalthis_default, r as init_globalthis } from "../_libs/@libsql/client+[...].mjs";
import { c as RouterProvider, n as defineHandlerCallback, t as renderRouterToStream } from "../_libs/@tanstack/react-router+[...].mjs";
import "../_libs/@inlang/paraglide-js+[...].mjs";
import processModule from "node:process";
import { AsyncLocalStorage } from "node:async_hooks";
//#region node_modules/.nitro/vite/services/ssr/index.js
var ssr_exports = /* @__PURE__ */ __exportAll({
	a: () => localizeHref,
	c: () => getClientIpFromCtx,
	d: () => guessLocaleFromAcceptLanguage,
...
async function getStartManifest(matchedRoutes) {
	const { tsrStartManifest } = await import("../_tanstack-start-manifest_v-DfGCMHs6.mjs");
...
var manifest = {
	"139369ba08645c08201ff69bd24b596cdc0f7784b8b3ffd8c6a14f47bbeac9cf": {
		functionName: "getBlogPostsFn_createServerFn_handler",
		importer: () => import("./server-CPJM4v-Y.mjs")
	},
	"b19d1425e0192068330144e802eeba64d1a59e4653703cc76476349d6a195d71": {
		functionName: "getBlogPostFn_createServerFn_handler",
		importer: () => import("./server-CPJM4v-Y.mjs")
	},
	"cbd81923ae1249d4c007f69579597cf1e79f1f7b8675cc608ba82e8926b5bb59": {
		functionName: "getAnalyticsConfigs_createServerFn_handler",
		importer: () => import("../__root-HTZjVkFr.mjs")
	}
};
...
async function loadEntries() {
	const [routerEntry, startEntry, pluginAdapters] = await Promise.all([
		import("./router-D1RahnsZ.mjs"),
		import("./start-vmDnqaSo.mjs"),
		import("./empty-plugin-adapters-D-2cGuKF.mjs")
	]);
	return {
		routerEntry,
		startEntry,
...
function getHeaderValue(ctx, headerName) {
	const h = ctx?.headers ?? ctx?.request?.headers;
	if (!h) return void 0;
	if (typeof h.get === "function") return h.get(headerName) ?? void 0;
	if (typeof h === "object") return h[headerName] ?? h[headerName.toLowerCase()] ?? void 0;
}
function getCookieFromCtx(ctx, name) {
	if (typeof ctx?.getCookie === "function") {
		const v = ctx.getCookie(name);
		if (typeof v === "string") return v;
	}
	return getCookieFromHeader(getHeaderValue(ctx, "cookie"), name);
}
function getClientIpFromCtx(ctx) {
	const forwarded = getHeaderValue(ctx, "x-forwarded-for");
	if (forwarded) {
		const first = forwarded.split(",")[0]?.trim();
		if (first) return first;
	}
	return getHeaderValue(ctx, "cf-connecting-ip") || getHeaderValue(ctx, "x-real-ip") || "";
}
function guessLocaleFromAcceptLanguage(acceptLanguage) {
	if (!acceptLanguage) return "";
	return ((acceptLanguage.split(",")[0]?.trim() ?? "").split("-")[0]?.trim() ?? "").replace(/[^\w-]/g, "").slice(0, 10);
}
/**
...
var getLocale = () => {
	...
	const resolved = resolveLocaleWithStrategies(strategy, typeof window !== "undefined" ? window.location?.href : void 0);
	if (resolved) {
...
var extractLocaleFromRequestWithStrategies = (request, strategies, url = request.url) => {
	...
	for (const strat of strategies) {
		if (strat === "cookie") locale = request.headers.get("cookie")?.split("; ").find((c) => c.startsWith("PARAGLIDE_LOCALE="))?.split("=")[1];
...
function extractLocaleFromUrl(url) {
	...
	let result;
	{
		...
		for (const element of urlPatterns) {
			for (const [locale, localizedPattern] of element.localized) if (new URLPattern(localizedPattern, urlObj.href).exec(urlObj.href)) {
				result = locale;
				break;
			}
			if (result) break;
...
function localizeUrl(url, options) {
	...
	for (const element of urlPatterns) {
		for (const [, localizedPattern] of element.localized) {
			...
			return fillMissingUrlParts(fillPattern(targetPattern, aggregateGroups(match), urlObj.origin), match);
		}
		const unlocalizedMatch = new URLPattern(element.pattern, urlObj.href).exec(urlObj.href);
		if (unlocalizedMatch) {
			const targetPattern = element.localized.find(([locale]) => locale === targetLocale)?.[1];
			if (targetPattern) return fillMissingUrlParts(fillPattern(targetPattern, aggregateGroups(unlocalizedMatch), urlObj.origin), unlocalizedMatch);
		}
	}
	return urlObj;
}
	...
	for (const element of urlPatterns) {
		for (const [, localizedPattern] of element.localized) {
			...
			if (match) {
				...
				return fillMissingUrlParts(fillPattern(element.pattern, groups, urlObj.origin), match);
			}
		}
		const unlocalizedMatch = new URLPattern(element.pattern, urlObj.href).exec(urlObj.href);
		if (unlocalizedMatch) return fillMissingUrlParts(fillPattern(element.pattern, aggregateGroups(unlocalizedMatch), urlObj.origin), unlocalizedMatch);
	}
	return urlObj;
}
/**
...
function deLocalizeHref(href) {
	...
	if (href.startsWith("/") && url.origin === deLocalized.origin) return deLocalized.pathname + deLocalized.search + deLocalized.hash;
...
var CF_WORKERS_MODULE = "cloudflare:workers";
var cfEnvPromise = null;
function ensureCloudflareEnv() {
	if (!cfEnvPromise) cfEnvPromise = import(
		/* @vite-ignore */
		CF_WORKERS_MODULE
).then((mod) => {
		globalThis.__CF_ENV__ = mod.env;
	}).catch(() => {});
	return cfEnvPromise;
}
var server_default = { async fetch(req) {
	await ensureCloudflareEnv();
	const response = await paraglideMiddleware(req, () => server_default$1.fetch(req));
	const utmSource = new URL(req.url).searchParams.get("utm_source");
	const existing = getCookieFromHeader(req.headers.get("cookie"), "utm_source");
	if (utmSource && !existing) {
		const sanitized = utmSource.replace(/[^\w.\-]/g, "").slice(0, 100);
		if (sanitized) response.headers.append("Set-Cookie", `utm_source=${sanitized}; Max-Age=2592000; Path=/; SameSite=Lax`);
	}
	return response;
} };
//#endregion
export { getClientIpFromCtx as a, getLocale as c, locales as d, localizeHref as f, ssr_exports as h, deLocalizeUrl as i, getServerFnById as l, setLocale as m, createServerFn as n, getCookieFromCtx as o, localizeUrl as p, deLocalizeHref as r, getHeaderValue as s, TSS_SERVER_FUNCTION as t, guessLocaleFromAcceptLanguage as u };
